...
<form action="test.php" method="post">
		<div id="example4">
			<label>Upload Lab report details:</label>
			<input type="text" class="form-control" name="project" placeholder="Report Name">
			<p></p>
			<input type="text" class="form-control" name="username" placeholder="Patient's Username">
			<p></p>
			<input class="btn btn-outline-primary" type="submit" name="submit" value="Upload"><br>
		</div>
		...
		else if ($_GET['status']=="done")
		{
            ...
            <br>
	<form action="test.php" method="POST">
		<button class="btn btn-outline-primary" type="submit" name="change">Report Completed</button>
	</form>
			<?php 
		}
	}
	else if (isset($_SESSION['user']) && $_SESSION['user']=="patient")
	{
		...
		$query = $db->query("SELECT * FROM images1 where username='$uid';");
